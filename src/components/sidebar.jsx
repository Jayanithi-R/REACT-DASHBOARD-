...
const SidebarProvider = React.forwardRef(
  ({ defaultOpen: defaultOpenProp = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
    ...
    const [open, setOpen] = React.useState(defaultOpenProp);

    const toggleSidebar = React.useCallback(() => {
        if (window.innerWidth < 768) {
            return setOpenMobile((v) => !v)
        } else {
            return setOpen((v) => !v)
        }
    ...
    const contextValue = React.useMemo(() => ({ state, open, setOpen, openMobile, setOpenMobile, toggleSidebar, hydrated }),
      [state, open, setOpen, openMobile, setOpenMobile, toggleSidebar, hydrated]
    );
...
const Sidebar = React.forwardRef(
  ({ className, children, ...props }, ref) => {
    const { state, openMobile, setOpenMobile, hydrated } = useSidebar();
